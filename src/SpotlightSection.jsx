...
function SpotlightSection() {
    ...
    const [APIStandardData , setAPIgetAllStandardData ] = useState([]);
    ...
    useEffect(() => {
      ...
      const getAllPosts = async () => {
        const res = await axios.get(`https://univisionz.in/aal/public/api/spotlight/16`)
    ...
    //  for the Standard Post Data 

      const getAllStandardPosts = async () => {
        const res = await axios.get(`https://univisionz.in/aal/public/api/standard/16`)
        const resp = res.data.data;
        console.log('resp')
        // console.log(resp)
        setAPIgetAllStandardData(resp);
      };

      useEffect(() => {
        console.log("shwo all data==>",APIStandardData)
        if (APIStandardData) {
          const filterData = !searchData
            ? APIStandardData
            : APIStandardData.filter((CurrentElement) =>
              CurrentElement.name.toLowerCase().includes(searchData.toLowerCase())
            );
          const perPageData = filterData.slice(offset - 1, offset - 1 + postsPerPage)
          const getStandardData = getStandardPostData(perPageData)
    
          setDisplayAllPost(getStandardData)
          setPageCount(Math.ceil(filterData.length / postsPerPage))
        }
    
      }, [APIStandardData , searchData])
    
      const handlePageClick = (event) => {
        const selectedPage = event.selected;
        setOffset(selectedPage + 1)
      };
// Till here Standard Post Data
  
const getPostData = (data) => {
    <h3>Spotlight Communities</h3>
    return (
 ...
 <div className='row flex-column'>
  ...
  const getStandardPostData = (data) => {
